      ...
      try {
        await runTransaction(db, async (transaction) => {
          ...
          if (GM_CODES.includes(secretCode)) {
            ...
            const firstThreeSnap = await getDoc(firstThreeRef);
            let gmEntered = firstThreeSnap.exists() ? firstThreeSnap.data().gmEntered || [] : [];

            if (!gmEntered.includes(secretCode)) {
              gmEntered.push(secretCode);
              await setDoc(firstThreeRef, { name: user.displayName, gmEntered }, { merge: true });
            }

            if (gmEntered.includes('GM1-0w4h') && gmEntered.includes('GM2-NF7k')) {
              const firstTwoRef = doc(db, 'firstTwo', user.uid);
              const firstTwoSnap = await getDoc(firstTwoRef);
              if (!firstTwoSnap.exists()) {
                await setDoc(firstTwoRef, { name: user.displayName, gmEntered });
              }
            }
          }
        });
